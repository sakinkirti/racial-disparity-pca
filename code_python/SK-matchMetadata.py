...
main_path = '/Users/sakinkirti/Programming/Python/CCIPD/racial-disparity-pca/dataset/to_nifti_cleaned'

def main(type):
    if type == 'single':
        img_path = glob.glob(f'{str(main_path)}/T2W.nii.gz', recursive=True)
        mask_paths = glob.glob(f'{str(main_path)}/LS*.nii.gz', recursive=True)
        ...
        for dir in subdirs:
            ...
            mask_paths = glob.glob(f'{str(dir)}/LS*.nii.gz', recursive=True)
