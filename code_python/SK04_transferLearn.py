...
def train(model, arch, modelname, device, num_epochs, learning_rate, weight_decay, patience, dataLoader, dataLabels, cv, rad):
    '''
...
def main():
    ...
    for rad in rads:
        ...
        # load the data
        splitspath = f'{source_dir}/model-outputs-{race}/rdp-hdf5/hdf5-{rad}'
        ...
        # train the model
        data_display, model = train(model, arch, modelname, device, num_epochs, learning_rate, weight_decay, patience, dataLoader, dataLabels, cv, rad)
        print('done')

        # generate some figures
        generate_figures(data_display, filetype='jpg', rad=rad, save_loc=f'{source_dir}/model-outputs-{race}/figures')
