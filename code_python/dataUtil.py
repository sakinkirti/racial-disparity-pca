...
class DataUtil(object):
    ...
    def getSubFiles(dir):
        dirpath = Path(dir)
        subfiles = [x for x in dirpath.iterdir() if x.is_file()]
        return subfiles
    ...
    @staticmethod
    def extract_bboxes(mask, dim=3):
        """Compute bounding boxes from masks.
        mask: [num_instances, height, width]. Mask pixels are either 1 or 0.
        dim (int): input dimensions of mask array
        Returns: bbox array [num_instances, (y1, x1, y2, x2)].
        """
        boxes = np.zeros(mask.shape, dtype=np.int32)
        if dim==3:
