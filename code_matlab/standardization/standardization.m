...
template_mask_T2W = niftiread([templatePath filesep 'PM_T2W_template.nii']);
template_mask_ADC = niftiread([templatePath filesep 'PM_ADC_template.nii']);

templateVolMasked_T2W = double(template_T2W) .* double(template_mask_T2W);
templateVolMasked_ADC = double(template_ADC) .* double(template_mask_ADC);
...
for i = 1:size(patients,1)-1
    ...
    % read the prostate mask
    mask = niftiread([datapath filesep patients{i} filesep 'PM.nii.gz']);

    % store the metadata to be rewritten to the standardized files
    hdr_T2W = niftiinfo([datapath filesep patients{i} filesep 'T2W.nii.gz']);
    hdr_ADC = niftiinfo([datapath filesep patients{i} filesep 'ADC_reg.nii.gz']);
    ...
    niftiwrite(T2Wstd, [datapath filesep patients{i} filesep 'T2W_std.nii'], hdr_T2W, 'Compressed',true); 
    niftiwrite(ADCstd, [datapath filesep patients{i} filesep 'ADC_std.nii'], hdr_ADC, 'Compressed',true);
end
